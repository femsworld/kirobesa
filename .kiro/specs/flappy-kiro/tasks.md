...
- [x] 3. Implement PhysicsEngine
  ...
  - [x] 3.2 Write property test for gravity update (Property 2)
    - **Property 2: Physics gravity update**
    - **Validates: Requirements 3.1, 3.3**
  - [x] 3.3 Write property test for flap override (Property 3)
    - **Property 3: Flap overrides velocity**
    - **Validates: Requirements 3.2**
  - [x] 3.4 Write property test for terminal velocity clamp (Property 4)
    - **Property 4: Terminal velocity clamp**
    - **Validates: Requirements 3.4**

- [x] 4. Implement PipeManager with object pooling
  - [x] 4.1 Write `PipeManager` class with pre-allocated pool of `PIPE_POOL_SIZE` pipe objects
  ...
  - [x] 4.2 Write property test for pipe scrolling and culling (Property 5)
    - **Property 5: Pipe scrolling and culling**
    - **Validates: Requirements 4.3, 4.4**
  - [x] 4.3 Write property test for pipe gap bounds (Property 6)
...
- [x] 5. Implement CollisionDetector
  ...
  - [x] 5.2 Write property test for collision detection (Property 7)
...
- [x] 6. Implement ScoreManager
  ...
  - [x] 6.2 Write property test for score increment (Property 8)
    - **Property 8: Score increments on pipe pass**
    - **Validates: Requirements 6.1**
  - [x] 6.3 Write property test for high score persistence (Property 9)
    - **Property 9: High score persistence**
    - **Validates: Requirements 6.3**
  - [x] 6.4 Write unit tests for ScoreManager
...
- [x] 8. Implement ParallaxSystem
  - [x] 8.1 Write `ParallaxSystem` class that initializes two `CloudLayer` objects from `CLOUD_LAYERS` config
  ...
  - [x] 8.2 Write property test for cloud depth ordering (Property 11)
    - **Property 11: Cloud layer depth ordering**
    - **Validates: Requirements 11.3, 11.4**
  - [x] 8.3 Write property test for cloud wrapping (Property 12)
    - **Property 12: Cloud wrapping**
    - **Validates: Requirements 11.5**
  - [x] 8.4 Write property test for clouds scrolling in all states (Property 13)
    - **Property 13: Clouds scroll in all states**
    - **Validates: Requirements 11.3, 11.6**

- [x] 9. Implement AudioManager
  - [x] 9.1 Write `AudioManager` class that loads `jump.wav` and `game_over.wav` via `HTMLAudioElement`
  ...
  - [x] 9.2 Write unit tests for AudioManager
    - Test `playJump()` calls `play()` on the correct audio element
    - Test `playGameOver()` calls `play()` on the correct audio element
    - _Requirements: 8.1, 8.2_

- [x] 10. Implement asset loader
  - [x] 10.1 Write `loadAssets()` function that returns a Promise resolving when `ghosty.png` is loaded
    - Fall back to drawing a colored rectangle if the image fails to load
    - _Requirements: 9.2_
  - [x] 10.2 Write unit tests for asset loader
...
- [x] 11. Implement Renderer
  - [x] 11.1 Write `Renderer` class with `drawStartScreen`, `drawPlayFrame`, and `drawGameOver` methods
  ...
  - [x] 11.2 Write unit tests for Renderer
...
- [x] 12. Implement game loop and state machine
  - [x] 12.1 Wire all subsystems into the main game loop in `game.js`
  ...
  - [x] 12.2 Write property test for state transitions (Property 1)
    - **Property 1: State transition on flap**
    - **Validates: Requirements 2.4, 2.5**
  - [x] 12.3 Write property test for frozen game over state (Property 10)
    - **Property 10: Frozen state in game over**
    - **Validates: Requirements 7.3**

- [x] 13. Implement responsive canvas resize
  ...
  - [x] 13.2 Write unit tests for canvas resize
    - Test that resize handler updates `canvas.width` and `canvas.height` to match window dimensions
    - _Requirements: 10.1_
  - [x] 13.3 Write property test for layout proportionality on resize (Property 14)
